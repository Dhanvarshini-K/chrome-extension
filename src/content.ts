...
(async () => {
  ...
  switch (engine) {
    ...
    //   // ==> took codeblock
    //   container = document.querySelector(
    //       ".ease-out.duration-200.relative.flex.w-full.flex-1.overflow-x-auto.overflow-y-scroll"
    //   )as HTMLElement | null; 
...
case "ClaudeO": {
  ...
  if (iframe) {
    ...
    if (iframeDoc) {
      // 👇 You embedded your artifact using this ID inside the iframe
      container = iframeDoc.getElementById("artifacts-component-root-html") as HTMLElement | null;

      if (!container) {
        console.warn("⚠️ Claude artifact container not found in iframe.");
      }
    } else {
      console.warn("⚠️ Cannot access Claude iframe contentDocument.");
    }
  } else {
    console.warn("⚠️ Claude iframe not found.");
  }
...
// declare global {
//   interface Window {
//     captureMode?: "message" | "code" | "both";
//   }
// }
// const mode = window.captureMode ?? "both";
